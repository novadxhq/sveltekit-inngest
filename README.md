...
When enabled, authorization is re-run for each message using that message's topic (`topics: [message.topic]`) right before emit. If authorization denies, throws, or message topic is invalid, the stream fails closed (degraded health event + close).
