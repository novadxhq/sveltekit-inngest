<script lang="ts">
	...
	import {triggerNestedInngestDemo} from "$demo/remote/demo.remote.js";
	import {getRealtimeState, getRealtimeTopicState} from "$lib/index.js";
	import {toast} from "svelte-sonner";

	const {health} = getRealtimeState();
	const message = getRealtimeTopicState<typeof secondDemoChannel, "message">(
		"message",
	);
...
<div class="grid gap-4">
	...
	<Button
		onclick={async () => {
			await triggerNestedInngestDemo(something);
			toast("triggered nested demo");
		}}
	>
		Trigger Nested Demo
	</Button>
