<script lang="ts">
  ...
  let liveListenerLog = $state<
    {
      id: number;
      topic: "message" | "admin-message";
      text: string;
      receivedAt: number;
    }[]
  >([]);

  const pushLiveListenerEntry = (
    topic: "message" | "admin-message",
    text: string,
  ) => {
    liveListenerLog = [
      {
        id: Date.now() + liveListenerLog.length,
        topic,
        text,
        receivedAt: Date.now(),
      },
...
<div class="grid gap-6">
  ...
  <div class="grid gap-4 lg:grid-cols-2">
    ...
    <Card>
      <CardHeader>
        <CardTitle>Listener Activity (`onMessage`)</CardTitle>
        <CardDescription>
          Live-only listener callbacks for <code>message</code> and
          <code>admin-message</code>. This log does not replay the latest
          retained state on mount.
        </CardDescription>
      </CardHeader>
      <CardContent>
        {#if liveListenerLog.length > 0}
          <div class="grid gap-2">
            {#each liveListenerLog as entry (entry.id)}
              <div class="rounded-lg border bg-muted/40 p-3">
                <div class="flex items-center justify-between gap-3">
                  <p class="text-sm font-medium">{entry.topic}</p>
                  <p class="text-xs text-muted-foreground">
                    {new Date(entry.receivedAt).toLocaleTimeString()}
                  </p>
                </div>
                <p class="mt-2 text-sm text-muted-foreground">{entry.text}</p>
              </div>
            {/each}
          </div>
        {:else}
          <p class="text-sm text-muted-foreground">
            Waiting for new listener callbacks...
          </p>
        {/if}
      </CardContent>
    </Card>
